...
- [ ] [HARD — decision gate] Namespace the frontmatter key `ocr_chars` → `scan_ocr_chars` (sibling gap left by id:874c) — ✅ DECISION-GATE RESOLVED (human 2026-07-11, relay human): rename IN PLUGIN `ocr_chars`→`scan_ocr_chars` (frontmatter emission only), following the id:874c flat-namespacing precedent; NOT core-side registration. Same migration posture as 874c (key already present in existing scan stores, accepted). Now executor-actionable. (was HARD/decision-gate, 🚧 GATED auto id:3801 route:decision-gate — un-gated by this decision.) — 🚧 GATED (auto, id:3801; route:decision-gate): aae8 done-check also blocked by unregistered 'original' frontmatter key (shared by zkm-pdf/zkm-eml/zkm-photo, not core-owned) — needs a cross-repo core-registry ruling, not a zkm-scan-local rename. — needs a /meeting <!-- id:1686 -->
