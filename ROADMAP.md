...
- [ ] [HARD — decision gate] Namespace frontmatter key `ocr_chars` → `scan_ocr_chars` (frontmatter emission only; id:874c flat-namespacing precedent; existing scan stores already carry the key, migration accepted). Decision-gate resolved 2026-07-11 (human). <!-- id:1686 --> — 🚧 GATED (auto, id:3801; route:decision-gate): aae8 Done-check needs `original` core-registered in zkm core (id:df3e, folded into f421, not yet landed in another repo) — needs reviewer decision on how to re-scope id:1686's Done-check or wait for f421. — needs a /meeting
