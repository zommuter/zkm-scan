...
- [ ] Namespace the frontmatter key `ocr_chars` → `scan_ocr_chars` (sibling gap left by id:874c) [HARD — decision gate] — 🚧 GATED (auto, id:3801; route:decision-gate): Done-check (aae8 test) also requires the shared `original` frontmatter key to pass core conformance; needs a cross-repo decision (core-owned registry vs. per-plugin namespacing) before this item can land green. — needs a /meeting <!-- id:1686 -->
